...
int pbar_anim_manual(void *data) {
	pbar_data *p = (pbar_data*) data;
	SDL_Rect src_r = {2, 0, gngeo_logo->w, gngeo_logo->h};
	SDL_Rect dst_r = {219 + 26, 146 + 16, gngeo_logo->w, gngeo_logo->h};
	SDL_Rect dst2_r = {0, 0, gngeo_logo->w, gngeo_logo->h};
...
void gn_init_pbar(char *name, int size) {
	...
	pbar.size = size;
	pbar.running = 1;
//	pbar_th = SDL_CreateThread(pbar_anim_thread, "PBAR THREAD",(void*) &pbar);
}

void gn_update_pbar(int pos) {
	pbar.pos = pos;
    
    pbar_anim_manual(&pbar);
}
...
int rom_browser_menu(void) {
	...
	if (init == 0) {
		...
		scaning = 1;
		//anim_th = SDL_CreateThread(rom_browser_scanning_anim, "ROMBROWSER THREAD", NULL);
...
static int dodump_action(GN_MENU_ITEM *self, void *param) {
	char *game = (char*) self->arg;
    char dump[8+4+1];    
	printf("Dumping %s\n", game);
    sprintf(dump,"%s.gno",game);
    
    close_game();
	if (conf.sound) close_sdl_audio();
    
    CF_BOOL(cf_get_item_by_name("dump"))=1;
    cf_get_item_by_name("dump")->flags |= CF_SETBYCMD;    
            
    if (init_game(game)!=SDL_TRUE) {
		gn_popup_error("Error! :", "Gngeo Couldn't init %s, dump was cancelled: \n\n%s"                       
                       , game,romerror);
        
		return MENU_STAY;
    }
    
    if (!dr_save_gno(&memory.rom,dump)) {
		gn_popup_error("Error! :", "Gngeo Couldn't dump %s: \n\n%s"                       
                       , game,romerror);
        close_game();
		return MENU_STAY;
    }
...
void init_rom_dumper_menu(void) {
	int i;
	int nbf;
	char filename[strlen(CF_STR(cf_get_item_by_name("rompath"))) + 256];
	struct stat filestat;
	...
	//char name[32];
	int nb_roms = 0;
	rdumper_menu = create_menu("Dump Game", MENU_SMALL, NULL, NULL);
	...
	while (romlist[i]) {
		...
		if (stat(filename, &filestat) == 0 && S_ISREG(filestat.st_mode)) {
			if ((drv = dr_check_zip(filename)) != NULL) {
				rdumper_menu->item = list_insert_sort(rdumper_menu->item,
                                                       (void*) gn_menu_create_item(drv->longname, MENU_ACTION, dodump_action, strdup(drv->name)),
                                                       romnamesort
                                                       );
				rdumper_menu->nb_elem++;
	...
	if (nb_roms == 0) {
		rdumper_menu->item = list_append(rdumper_menu->item,
                                          (void*) gn_menu_create_item("No Games Found...", MENU_ACTION, NULL, NULL));
		rdumper_menu->nb_elem++;
	}
}

int rom_dumper_scanning_anim(void *data) {
	int i = 0;
	while (scaning) {
		draw_back();
		if (i > 20)
...
int rom_dumper_menu(void) {
	...
	if (init == 0) {
		...
		scaning = 1;
		//anim_th = SDL_CreateThread(rom_dumper_scanning_anim, "ROMDUMPER THREAD", NULL);
		init_rom_dumper_menu();
		scaning = 0;
		//SDL_WaitThread(anim_th, NULL);
	}
...
static int rdumper_action(GN_MENU_ITEM *self, void *param) {
	//exit(0);
	return rom_dumper_menu();
}
...
/*    main_menu->item = list_append(main_menu->item,
            (void*) gn_menu_create_item("Dump game", MENU_ACTION, rdumper_action, NULL));
	main_menu->nb_elem++;
*/
	main_menu->item = list_append(main_menu->item,
